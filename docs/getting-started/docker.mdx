...
:::info
An alternative Docker image is available on Docker Hub for this project. You can find it at [Docker Hub Repository Link](https://hub.docker.com/r/snapetech/seerrng)

Our Docker images are available with the following tags:

- `latest`: Always points to the most recent stable release.
- Version tags (e.g., `v0.1.0`): For specific stable versions.
- Major version aliases (e.g., `v0`): Alias for the latest stable release in the respective major version series.
- Minor version aliases (e.g., `v0.1`): Alias for the latest stable release in the respective minor version series.
:::
...
:::note Authentication transport
The default container listener on port 5055 serves HTTP, but authenticated
session cookies remain HTTPS-only. For direct LAN HTTPS without Caddy, Traefik,
or another proxy, follow the [built-in HTTPS guide](/using-seerr/advanced/built-in-tls)
and publish port 5056 as well. The first-run page can save the self-signed or
provided-certificate choice before the first login; restart the container to
activate it. The explicit `SEERR_ALLOW_HTTP_AUTH=true` fallback is available
for trusted LANs that cannot use TLS, but requires a deliberate risk
acknowledgement when configured from the UI.
:::
...
docker run -d \
  ...
  --health-cmd "node /app/scripts/healthcheck.mjs" \
...
```bash
docker pull ghcr.io/snapetech/seerrng:latest
```
...
services:
  seerr:
    ...
    healthcheck:
      test: node /app/scripts/healthcheck.mjs
      start_period: 20s
      timeout: 3s
      interval: 15s
      retries: 3
